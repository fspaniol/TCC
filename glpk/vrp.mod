...
# Check whether arch A from flow F is handled by route K

param q;
# Capacity that each group an handle

minimize groups: sum{v in V, k in K, s in S} X[0,v,k,s];
# amount of telemetry submissions
...
# This binds the route to go back to the base router

s.t. bindFlow{k in K, s in S, (u,v) in A diff F[s]}: X[u,v,k,s] = 0;
# A link can only be handled by a flow that has it
